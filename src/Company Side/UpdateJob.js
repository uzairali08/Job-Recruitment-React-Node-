...
function UpdateJob() {
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      if (jobId) {
        try {
          const response = await fetch(`http://localhost:5000/api/EditJob/${jobId}`);
          const data = await response.json();
          // setJob(data.job);
          setJob_Title(data.job.job_Title);
          setJob_Location(data.job.job_Location);
          setJob_Type(data.job.job_Type);
          setJob_PeopleLimit(data.job.job_PeopleLimit);
          setJob_Deadline(data.job.job_Deadline);
          setJob_PayRange(data.job.job_PayRange);
          setJob_PayMin(data.job.job_PayMin);
          setJob_PayMax(data.job.job_PayMax);
          setJob_PayRate(data.job.job_PayRate);
          setJob_Desc(data.job.job_Desc);
          setJob_ResumeStatus(data.job.job_ResumeStatus);
          setJob_DeadlineStatus(data.job.job_DeadlineStatus);
          setJob_RejectionStatus(data.job.job_RejectionStatus);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/UpdateJob/${jobId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const data = await response.json();
      console.log(data); // Assuming the API responds with { success: true }
